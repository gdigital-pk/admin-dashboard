    ...
    <main className="relative min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-200 via-white to-pink-200 p-4 overflow-hidden">
      <div className="absolute inset-0 z-0 pointer-events-none">
        <svg className="w-full h-full opacity-30" viewBox="0 0 700 500">
          <defs>
            <radialGradient id="loginGradient1" cx="30%" cy="20%" r="90%" gradientTransform="rotate(25)">
              <stop offset="0%" stopColor="#a5b4fc" />
              <stop offset="100%" stopColor="#f0abfc" />
            </radialGradient>

            <radialGradient id="loginGradient2" cx="70%" cy="80%" r="80%" gradientTransform="rotate(-25)">
              <stop offset="0%" stopColor="#f472b6" />
              <stop offset="100%" stopColor="#c7d2fe" />
            </radialGradient>
          </defs>

          <ellipse cx="350" cy="250" rx="360" ry="230" fill="url(#loginGradient1)" />

          <ellipse cx="580" cy="380" rx="200" ry="140" fill="url(#loginGradient2)"
          />
        </svg>
      </div>

      <form onSubmit={onSubmit} noValidate className="relative z-10 w-full max-w-md space-y-7 bg-white/80 backdrop-blur-md border-2 border-white/80 rounded-2xl shadow-xl shadow-indigo-200 px-8 py-10 transition duration-300 hover:scale-[1.015]">
        ...
        <div className="flex flex-col items-center mb-4">
          <span className="inline-flex items-center justify-center w-20 h-20 rounded-full bg-indigo-100 border-2 border-indigo-300 mb-3 shadow-lg animate-bounce-slow">
            <svg width="44" height="44" viewBox="0 0 24 24" fill="none">
              ...
              />
              <path d="M9.5 12L11.2 13.7L14.8 10.1" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </span>
        ...
        <div className="space-y-2">
          ...
          <input type="email" value={email} onChange={handleChange("email")} placeholder="Enter your email address" autoComplete="email"
            required className={`w-full rounded-lg border-2 bg-white/90 px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-indigo-400 transition ${errors.email
              ? "border-pink-500" : "border-indigo-200"}`} />
        ...
        <div className="space-y-2">
          ...
          <input type="password" value={password} onChange={handleChange("password")} placeholder="Enter your password" autoComplete="current-password" required className={`w-full rounded-lg border-2 bg-white/90 px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-indigo-400 transition ${errors.password
            ? "border-pink-500" : "border-indigo-200"}`} />
        ...
        <button disabled={loading} className="w-full rounded-xl bg-gradient-to-r from-indigo-500 via-violet-500 to-pink-500 px-6 py-3 text-lg font-bold text-white shadow hover:scale-105 transition-transform duration-150 focus:outline-none focus:ring-2 focus:ring-pink-300 disabled:from-indigo-300 disabled:via-rose-200 disabled:to-pink-300 disabled:cursor-not-allowed">
          {loading ? (
            <span className="flex items-center justify-center gap-2">
              <svg className="w-5 h-5 animate-spin text-white/90" fill="none" viewBox="0 0 24 24">
